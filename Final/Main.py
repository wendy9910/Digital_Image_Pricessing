...
def get_value(value):
    print(value)
    print('eyescale1 set to',eyescale1.get())
...
eyescale1 = tk.Scale(
    block2, label='大小', from_=0, to=100, orient="horizontal"
    ,tickinterval=10,length=280)
eyescale1.bind("<ButtonRelease-1>", get_value)

eyescale2 = tk.Scale(
    block2, label='旋轉', from_=0, to=100, orient="horizontal"
    ,tickinterval=10,length=280,command=get_value)
eyescale3 = tk.Scale(
    block2, label='眼距', from_=0, to=100, orient="horizontal"
    ,tickinterval=10,length=280,command=get_value)
